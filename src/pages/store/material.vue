...
const columns = [
  ...
  { name: 'who_get', align: 'left',  label: 'Кол-во', field: row => row.amount ,  sortable: true},
  { name: 'give_serial_numbers', align: 'left',  label: 'Серийные номера', field: row => row.give_serial_numbers ,  sortable: true},
...
const getItem = async () => {
  ...
  const response = await api(`/api/material_store/store_material/${route.params.id}?full=true`)
...
<q-page padding>
  ...
  <div class="row q-col-gutter-md">
    ...
    <div class="col-6">
      <p class="no-margin">Поступило на склад: {{item.init_amount}}</p>
      <p class="no-margin">Остаток на складе: {{item.amount}}</p>
      <p class="no-margin">Серийные номера на складе:  </p>
      <pre class="q-mb-lg">{{item.serial_numbers}}</pre>
    </div>
  </div>
